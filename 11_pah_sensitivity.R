...
impute_one_pah_envstats <- function(x, lod) {
  ...
  # PAH-level beta using a reference LOD (median among censored rows)
  lod_ref <- stats::median(lod_ok[censored], na.rm = TRUE)
...
calc_beta_envstats <- function(x, lod) {
  ...
  lod_ref <- stats::median(lod_ok[censored & lod_ok > 0], na.rm = TRUE)
