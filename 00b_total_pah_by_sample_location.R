...
valid_conditions <- group_summary %>%
  ...
  pull(Sample_Location)

# --- 2. Filter dataset ---
filtered_data <- data_fabric_raw %>%
  filter(Sample_Location %in% valid_conditions)
...
valid_pairs <- map_dfr(condition_pairs, function(pair) {
  x <- na.omit(filtered_data$`Total_PAH_ug/g`[filtered_data$Sample_Location == pair[1]])
  y <- na.omit(filtered_data$`Total_PAH_ug/g`[filtered_data$Sample_Location == pair[2]])
...
pairwise_res <- map_dfr(1:nrow(valid_pairs), function(i) {
  x <- na.omit(filtered_data$`Total_PAH_ug/g`[filtered_data$Sample_Location == valid_pairs$group1[i]])
  y <- na.omit(filtered_data$`Total_PAH_ug/g`[filtered_data$Sample_Location == valid_pairs$group2[i]])
...
plot_pairwise <- pairwise_res %>%
  left_join(y_max, by = c("group1" = "Sample_Location")) %>%
  rename(y1 = ymax) %>%
  left_join(y_max, by = c("group2" = "Sample_Location")) %>%
  rename(y2 = ymax) %>%
...
ggplot(filtered_data, aes(x = Sample_Location, y = `Total_PAH_ug/g`, fill = Sample_Location)) +
  ...
  labs(
    title = "Total PAH by Sample Location",
